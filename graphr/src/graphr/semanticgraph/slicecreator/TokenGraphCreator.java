 ...
 * @author Florian
 */
public class TokenGraphCreator implements SemanticSliceCreator {

    @Override
    public SemanticGraph createSemanticSliceGraph(SemanticGraph graph, String dataSource) {
        SemanticSliceGraph<GHT, GHT> semanticSlice = new SemanticSliceGraph<GHT, GHT>(SemanticSliceGraphType.TOKEN);
        ...
        for (Tupel<String, String> strToken : strTokens) {
            SemanticVertex tokenVertex = new SemanticVertex(SemanticSliceGraphType.TOKEN.getSliceName());
        ...
        for (int i = 0; i < tokens.size() - 1; i++) {
            ...
            semanticSlice.addEdge(curr, next, SemanticSliceGraphType.TOKEN.getIncommingEdgeName());
        }
